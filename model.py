...
def build_gen0(h1, z0):
    gen0_z_embed1 = tf.layers.batch_normalization(
                        tf.nn.relu(utils.dense(z0, num_inputs=16, num_units=128, bias=True,
                            name='gen0_z_embed1')))
    gen0_z_embed2 = tf.layers.batch_normalization(
                        tf.nn.relu(utils.dense(gen0_z_embed1, num_inputs=128, num_units=128, bias=True,
                            name='gen0_z_embed2')))
    ...
    gen0_in_reshaped = tf.reshape(
                        tf.layers.batch_normalization(
                                tf.nn.relu(utils.dense(gen0_in, num_inputs=384, num_units=256*5*5,
                                    name='gen0_embed'))), [-1, 5, 5, 256])

    gen0_deconv1 = tf.layers.batch_normalization(
                        tf.nn.relu(utils.conv2d_transpose(gen0_in_reshaped, (5, 5, 256, 256), (100, 10, 10, 256),
                            stride=(1, 2, 2, 1), padding='SAME', name='gen0_deconv1')))
    gen0_deconv2 = tf.layers.batch_normalization(
                        tf.nn.relu(utils.conv2d_transpose(gen0_deconv1, (5, 5, 128, 256), (100, 14, 14, 128),
                            padding='VALID', name='gen0_deconv2')))

    gen0_deconv3 = tf.layers.batch_normalization(
                        tf.nn.relu(utils.conv2d_transpose(gen0_deconv2, (5, 5, 128, 128), (100, 28, 28, 128),
                            stride=(1, 2, 2, 1), padding='SAME', name='gen0_deconv3')))
    gen0_deconv4 = tf.layers.batch_normalization(
                        tf.nn.relu(utils.conv2d_transpose(gen0_deconv3, (5, 5, 3, 128), (100, 32, 32, 3),
